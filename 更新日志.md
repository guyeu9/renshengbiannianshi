...
- **地图功能 Android 15 兼容性修复**：
  - **根因**：2026-02-20 因高德 SDK `libAMapOpenMap.so` 在 Android 15 上触发 JNI 崩溃，临时禁用了 Android 15+ 的原生地图功能，导致地图无法显示、定位功能无效
  - **解决**：升级高德地图 SDK 从 `10.0.600` 到 `10.0.800`（Maven Central 可用版本），补充 Android 15 混淆规则，移除 Android 15 禁用逻辑，恢复原生地图体验
  - **变动文件**：
    - `android/app/build.gradle`：SDK 版本升级
    - `plugins/amap_flutter_map/android/build.gradle`：SDK 版本升级
    - `plugins/amap_flutter_location/android/build.gradle`：SDK 版本升级
    - `android/app/proguard-rules.pro`：补充 Android 15 混淆规则
    - `lib/core/widgets/amap_location_page.dart`：移除 Android 15 禁用逻辑与提示文案

 **地图功能 Android 15 兼容性修复尝试（方案 A 失败）**：
  - **根因**：2026-02-20 因高德 SDK `libAMapOpenMap.so` 在 Android 15 上触发 JNI 崩溃，临时禁用了 Android 15+ 的原生地图功能
  - **尝试方案 A**：尝试升级高德地图 SDK 到 `10.0.800`，但该版本在 Maven Central 等公共仓库不存在，构建失败
  - **当前状态**：回退到 `10.0.600`，保持 Android 15 禁用逻辑，用户需使用"手动填写"或"外部导航"
  - **后续方案**：需等待高德官方发布 Android 15 兼容的 SDK 版本，或采用方案 B（WebView 混合方案）
