...
- 已在 Drift 中落地：`food_records`、`moment_records`、`friend_records`、`travel_records`、`trips`、`entity_links`、`link_logs`、`timeline_events`。
- 相遇（encounter）当前落地策略：未引入 `encounter_records` 表；新建相遇写入 `timeline_events`（`eventType='encounter'`），并通过 `entity_links` 记录与朋友/美食等实体的关联。查询时使用 `watchEncountersForFriend` 通过 `entity_links` 反查。
...
- 迁移策略：当前 schemaVersion=4，v1→v2 增量迁移创建新增三实体表，v2→v3 增量迁移创建旅行相关表，v3→v4 增量迁移为 food/moment/travel/friend 追加 is_favorite 字段；首次安装直接 `createAll()` 一次建表。
...
- 羁绊（原型：朋友档案/相遇）：[bond_page.dart](file:///d:/trae/%E4%BA%BA%E7%94%9F%E7%BC%96%E5%B9%B4%E5%9B%BD%E9%99%85/life_chronicle/lib/features/bond/presentation/bond_page.dart)
  ...
  - 日期时间选择：朋友认识日期/生日、相遇记录时间均支持“日期+时间”选择，统一 `zh-CN` 语言环境
...
- 根因：`home_schedule_page.dart` 引用了 `intl/table_calendar` 但未在 `pubspec.yaml` 声明依赖；同时 AppTheme 引用路径写错（误指向不存在的 `core/theme/app_theme.dart`），导致 `AppTheme` 相关常量无法解析并触发大量 `invalid_constant/undefined_identifier`；另外遗漏导入 `appDatabaseProvider` 导致日程事件流无法解析
- 解决：
  - 依赖补齐：在 `pubspec.yaml` 增加 `intl` 与 `table_calendar` 依赖声明，确保包可被解析
...
- 变动：仅涉及依赖声明与导入路径修正，不涉及数据库表结构调整；相关文件：`home_schedule_page.dart`、`pubspec.yaml`、`widget_test.dart`
