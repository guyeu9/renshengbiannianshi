...
#### 4. AMap SDK 依赖冲突与修复
- **问题**：`android/app/build.gradle` 中同时引入了 `com.amap.api:3dmap:10.0.600` 和 `com.amap.api:search:9.7.1`，导致构建时出现重复类定义冲突 (`Duplicate class com.amap.apis.utils.core.api.AMapUtilCoreApi`)。
- **原因**：`3dmap` SDK 10.x 版本与 `search` SDK 9.x 版本包含不兼容的公共基础库 (`core-api`)，且项目实际逻辑中未使用原生 Search SDK（Dart 层使用 REST API）。
- **修复方案**：移除 `android/app/build.gradle` 中未使用的 `com.amap.api:search:9.7.1` 依赖。
