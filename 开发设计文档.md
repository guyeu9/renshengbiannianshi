...
- 解决：
  ...
  - Android 15+ 在应用内用高德“静态地图”接口展示地点预览图（`restapi.amap.com/v3/staticmap`），满足“可在应用内看到地图预览”的体验，同时不触发 native 库加载
  - 保留地点能力：预览页仍可“外部导航”；选择页仍可“搜索地点（Web API）/手动填写/选中写入经纬度”，确保各安卓版本不闪退且功能可用
  - 二手准备：曾尝试替换为 `amap_flutter_map`（官方插件方向），但该版本在当前 Flutter/Dart 环境下存在编译期问题（依赖中使用已移除的 `hashValues`），因此本次以“静态地图预览 + 原搜索能力”作为 Android 15 的稳定落地方案
- 变动：
  - Flutter：`amap_location_page.dart`（增加版本判定、Android 15+ 静态地图预览渲染、保留搜索/手动填写/外部导航）
