...
| 地图 SDK | 高德地图 Flutter 插件（Android SDK 10.0.800） | 国内场景，味觉地图 + 旅行足迹地图 [1]；Android 15 兼容性已验证
...
### 6.10 本次实现说明（2026-02-22 高德依赖构建修复）
...
### 6.46 本次实现说明（2026-02-23 地图功能 Android 15 兼容性修复）
...
- **解决**：
  - 升级高德地图 SDK 从 `10.0.600` 到 `10.0.800`（Maven Central 可用版本）
  - 补充 Android 15 混淆规则（`com.amap.api.maps.model.**`、`com.amap.api.maps.utils.**`、`com.amap.api.location.**`）
  - 移除 `amap_location_page.dart` 中的 Android 15 禁用逻辑与提示文案
  - 移除未使用的 `_androidMajorVersion()` 方法和 `dart:io` 导入
- **变动**：
  - `android/app/build.gradle`：SDK 版本 `10.0.600` → `10.0.800`
  - `plugins/amap_flutter_map/android/build.gradle`：SDK 版本 `10.0.600` → `10.0.800`
  - `plugins/amap_flutter_location/android/build.gradle`：SDK 版本 `10.0.600` → `10.0.800`
  - `android/app/proguard-rules.pro`：补充 Android 15 混淆规则
  - `lib/core/widgets/amap_location_page.dart`：移除 Android 15 禁用逻辑
