...
- 相遇（encounter）当前落地策略：未引入 `encounter_records` 表；新建相遇先写入 `timeline_events`（`eventType='encounter'`），并通过 `entity_links/link_logs` 记录与朋友/美食等实体的关联。
...
- 迁移策略：当前 schemaVersion=3，v1→v2 增量迁移创建新增三实体表，v2→v3 增量迁移创建旅行相关表；首次安装直接 `createAll()` 一次建表。
...
- 收藏中心已补齐概览统计、分类筛选与选择模式批量导出/删除入口，并接入本地数据库真实数据：food_records（标题/标签/图片/record_date）、travel_records（标题/目的地/心情/图片/record_date）、moment_records（内容/心情/场景/图片/record_date）、friend_records（姓名/分组/头像/updated_at），汇总后按时间倒序渲染；编年史管理已补齐列表与卡片落地；年度报告/数据管理/模块管理等入口仍以占位页承接
- 美食（原型：美食主页/新建/详情）：[food_page.dart](file:///d:/trae/%E4%BA%BA%E7%94%9F%E7%BC%96%E5%B9%B4%E5%9B%BD%E9%99%85/life_chronicle/lib/features/food/presentation/food_page.dart)
  ...
  - 详情：封面图、基础信息、记录内容、底部操作栏（心愿清单详情页也在同一落点承接）；万物互联改为读取 `entity_links` 动态展示关联人物/小确幸/旅行/目标
