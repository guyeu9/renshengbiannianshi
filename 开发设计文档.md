...
| 地图 SDK | 高德地图 Flutter 插件（Android SDK 10.0.600） | 国内场景，味觉地图 + 旅行足迹地图 [1]；Android 15 存在 JNI 兼容性问题，已禁用原生地图 |
...
### 6.46 本次实现说明（2026-02-23 地图功能 Android 15 兼容性修复尝试）
...
- **根因**：
  ...
  - 禁用后导致 Android 15 设备上地图无法显示、定位功能无效，仅能通过搜索（Web API）和外部导航使用地点功能
- **尝试方案 A（失败）**：
  - 尝试升级高德地图 SDK 到 `10.0.800`，但该版本在 Maven Central、阿里云 Maven 等公共仓库均不存在
  - 构建失败：`Could not find com.amap.api:3dmap:10.0.800`
- **当前状态**：
  - 已回退 SDK 版本到 `10.0.600`（Maven Central 可用版本）
  - 保持 Android 15 禁用逻辑，用户需使用"手动填写"或"外部导航"
- **后续方案**：
  - 等待高德官方发布 Android 15 兼容的 SDK 版本
  - 或采用方案 B（WebView 混合方案）
- **变动**：
